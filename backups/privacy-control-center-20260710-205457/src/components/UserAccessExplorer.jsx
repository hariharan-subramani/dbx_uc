...
function SearchUserPanel({ exportedBy, initialUser }) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      fetch(`${API_BASE}/users?search=${encodeURIComponent(query)}&fresh=true`, { signal: controller.signal })
        .then((response) => response.json())
        .then((data) => {
          setUsers(data.users || []);
          setMessageTone(data.success ? 'success' : 'error');
  ...
  const handleSearch = async (userOverride = '') => {
    ...
    setProfile(null);
    try {
      const data = await fetchJson(`/users/${encodeURIComponent(userToLoad)}/access?scan=true&fresh=true&_=${Date.now()}`);
      if (!data.success) {
        setProfile(null);
        setMessageTone('error');
        setMessage(data.message || 'Unable to load live user access.');
        return;
      }

      setProfile(data);
      setMessageTone('success');
      setMessage('User information loaded successfully.');
    } catch {
      setProfile(null);
      setMessageTone('error');
      setMessage('Unable to load live user access.');
    } finally {
  ...
  useEffect(() => {
    const refreshVisibleProfile = () => {
      const activeUser = profile?.user?.email || profile?.user?.name || selectedUser || query;
      if (activeUser) handleSearch(activeUser);
    };

    window.addEventListener('privacy-permissions-changed', refreshVisibleProfile);
    window.addEventListener('privacy-groups-changed', refreshVisibleProfile);
    return () => {
      window.removeEventListener('privacy-permissions-changed', refreshVisibleProfile);
      window.removeEventListener('privacy-groups-changed', refreshVisibleProfile);
    };
  }, [profile?.user?.email, profile?.user?.name, selectedUser, query]);
  ...
  return (
    <>
      ...
      {message && <div className={`alert ${messageTone}`} role={messageTone === 'success' ? 'status' : 'alert'}><span>{messageTone === 'success' ? '✓' : '!'}</span>{message}</div>}

      {loadingAccessScan && (
        <div className="scan-status">
          <div className="loading-spinner small"></div>
          <span>Scanning catalog, schema, table, volume, and privilege access...</span>
        </div>
      )}
      ...
      {profile && (
        <div className="identity-profile">
          ...
          <section className="access-overview">
            ...
            <AccessMetric label="Volumes" value={profile.volumes?.length || 0} />
          ...
          <CompactAccessList
            title="Volume Access"
            items={profile.volumes}
            emptyMessage="No volume access found."
            loading={loadingAccessScan}
          />
        </div>
      )}
