...
function Explorer({ user }) {
  const { workspace, catalog, schema, table, volume } = useParams();
  ...
  }, [workspace, catalog, schema, table, volume]);
  ...
  async function loadSchemaSelection(catalogName, schemaName) {
    ...
    try {
      const [metadataData, tablesData, volumesData] = await Promise.all([
        ...
        fetchJson(`/catalogs/${encodeURIComponent(catalogName)}/schemas/${encodeURIComponent(schemaName)}/volumes`).catch((err) => ({ success: false, error: err.message, volumes: [] })),
      ]);

      const nextTables = tablesData.tables || [];
      const nextVolumes = volumesData.volumes || [];
      setTables(nextTables);
      setVolumes(nextVolumes);
      if (!tablesData.success) setError('Tables are unavailable for this schema.');
      if (!volumesData.success) setError(volumesData.message || 'Volumes are unavailable for this schema.');
      ...
      setObjectDetails([
        ...
        { label: 'Volume Count', value: String(nextVolumes.length) },
        { label: 'Comment', value: meta.comment || 'Metadata comment is unavailable.' },
        { label: 'Created Date', value: formatDate(meta.created_at) },
      ]);
  ...
  async function loadVolumeSelection(catalogName, schemaName, volumeName) {
    setLoadingDetails(true);
    setSelectedObject({ type: 'volume', name: volumeName, catalogName, schemaName, workspaceName: selectedWorkspaceLabel() });
    setObjectDetails(null);
    setPermissions(null);
    setPermissionsMessage('');
    setLoadingPermissions(true);
    ...
    try {
      const metadataData = await fetchJson(
        `/catalogs/${encodeURIComponent(catalogName)}/schemas/${encodeURIComponent(schemaName)}/volumes/${encodeURIComponent(volumeName)}/metadata`
      ).catch((err) => ({ success: false, error: err.message }));

      const meta = metadataData.success ? metadataData.metadata || {} : {};
      setObjectDetails([
        { label: 'Name', value: meta.name || volumeName },
        { label: 'Owner', value: unavailable(meta.owner) },
        { label: 'Created Date', value: formatDate(meta.created_at) },
        { label: 'Volume Type', value: meta.volume_type || 'Unavailable' },
        { label: 'Storage Location', value: meta.storage_location || 'Unavailable' },
        { label: 'Comment', value: meta.comment || 'Metadata comment is unavailable.' },
      ]);

      fetchJson(`/volumes/${encodeURIComponent(catalogName)}/${encodeURIComponent(schemaName)}/${encodeURIComponent(volumeName)}/permissions`)
        .then((permissionsData) => {
          setPermissions(permissionsData.success ? permissionsData.permissions || [] : []);
          setPermissionsMessage(permissionsData.success ? '' : permissionsData.message || 'Unable to load permissions.');
        })
        .catch((err) => {
          setPermissions([]);
          setPermissionsMessage('Unable to load permissions.');
          console.error('Error loading volume permissions:', err);
        })
        .finally(() => setLoadingPermissions(false));
    } catch (err) {
      setError('Volume metadata is unavailable right now.');
      setLoadingPermissions(false);
      console.error('Error loading volume details:', err);
    } finally {
      setLoadingDetails(false);
    }
  }
  ...
  const handleSelectVolume = (volumeName) => {
    isNavigatingFromURL.current = true;
    setSelectedVolume(volumeName);
    setSelectedTable('');

    if (volumeName && selectedWorkspace && selectedCatalog && selectedSchema) {
      navigate(`/workspace/${encodeURIComponent(selectedWorkspace)}/catalog/${encodeURIComponent(selectedCatalog)}/schema/${encodeURIComponent(selectedSchema)}/volume/${encodeURIComponent(volumeName)}`);
    }
  };

  const refreshSelectedObject = async () => {
    if (selectedVolume && selectedCatalog && selectedSchema) {
      await loadVolumeSelection(selectedCatalog, selectedSchema, selectedVolume);
      return;
    }
