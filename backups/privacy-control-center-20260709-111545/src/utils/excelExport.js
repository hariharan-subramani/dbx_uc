...
export const exportGroupManagement = ({
  group,
  members,
  permissions,
  generatedBy,
  ...
  const groupDetails = [{
    ...
    'Number of Members': group?.member_count ?? members?.length ?? 0,
    'Group Type': group?.group_type || '',
    'Export Date': exportTimestamp(),
    'Generated By': generatedBy || '',
  }];

  appendSheet(workbook, 'Group Details', XLSX.utils.json_to_sheet(groupDetails));
  appendSheet(
    workbook,
    'Group Members',
    sheetFromRows((members || []).map((member) => ({
      User: member.display || member.email || member.name || member.value || '',
      Email: member.email || member.display || '',
      Role: member.role || 'Member',
      'User ID': member.value || member.id || '',
    })), 'No members available.'),
  );
  appendSheet(
    workbook,
    'Group Permissions',
    sheetFromRows(toPermissionRows(permissions), 'No group permissions available.'),
  );

  downloadWorkbook(workbook, `GroupManagement_${safeFilePart(group?.name)}.xlsx`);
};
