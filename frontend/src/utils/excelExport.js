...
const downloadWorkbook = (workbook, filename) => {
  try {
    ...
    console.error('[ExcelExport] Workbook download failed', {
      filename,
...
export const exportVolume = ({
  ...
  const detailMap = detailMapFrom(details);
  const volumeDetails = [{
    'Volume Name': detailMap.Name || volumeName || '',
    Owner: detailMap.Owner || '',
    'Created Date': detailMap['Created Date'] || '',
    'Volume Type': detailMap['Volume Type'] || '',
    'Storage Location': detailMap['Storage Location'] || '',
    Comment: detailMap.Comment || '',
    'Export Date': exportTimestamp(),
    'Exported By': exportedBy || '',
  }];

  appendSheet(workbook, 'Volume Information', XLSX.utils.json_to_sheet(volumeDetails));
  appendSheet(
    workbook,
    'Permissions',
    sheetFromRows(toPermissionRows(permissions), 'No permissions available.'),
  ...
  if ((bindings || []).length > 0) {
    appendSheet(
      workbook,
      'Binding Information',
      sheetFromRows(toCatalogBindingRows(bindings), 'No binding information available.'),
    );
  }

  downloadWorkbook(workbook, `Volume_${safeFilePart(volumeName)}.xlsx`);
};
...
const toGroupAccessRows = (items = []) => items.map((item) => ({
  Name: item.name || '',
  Catalog: item.catalog || '',
  Schema: item.schema || '',
  Table: item.table || '',
...
export const exportGroupAccess = ({ profile, exportedBy }) => {
  const workbook = createWorkbook();
  const group = profile?.group || {};
  const details = [{
    'Group Name': group.name || '',
    Description: group.description || '',
    'Principal Type': group.principal_type || 'Group',
    'Members Count': profile?.members?.length || 0,
    'Workspace Count': profile?.workspaces?.length || 0,
    'Catalog Count': profile?.catalogs?.length || 0,
    'Schema Count': profile?.schemas?.length || 0,
    'Table Count': profile?.tables?.length || 0,
    'Export Date': exportTimestamp(),
    'Exported By': exportedBy || '',
  }];

  appendSheet(workbook, 'Group Information', XLSX.utils.json_to_sheet(details));
  appendSheet(
    workbook,
    'Members',
    sheetFromRows((profile?.members || []).map((member) => ({
      Name: member.name || '',
      Email: member.email || '',
  ...
  appendSheet(
    workbook,
    'Workspace Access',
    sheetFromRows((profile?.workspaces || []).map((workspace) => ({
      'Workspace Name': workspace.display_name || workspace.name || workspace.host || '',
      'Workspace ID': workspace.workspace_id || '',
      Host: workspace.host || '',
    })), 'No workspace access available.'),
  );
  appendSheet(workbook, 'Catalog Access', sheetFromRows(toGroupAccessRows(profile?.catalogs), 'No catalog access available.'));
  appendSheet(workbook, 'Schema Access', sheetFromRows(toGroupAccessRows(profile?.schemas), 'No schema access available.'));
  appendSheet(workbook, 'Table Access', sheetFromRows(toGroupAccessRows(profile?.tables), 'No table access available.'));
  appendSheet(
    workbook,
    'Permissions',
    sheetFromRows((profile?.privileges || []).map((privilege) => ({ Privilege: privilege })), 'No permissions available.'),
  );

  downloadWorkbook(workbook, `Group_${safeFilePart(group.name)}.xlsx`);
};
